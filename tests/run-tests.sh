...
echo '[1/8] 规则逻辑单测（node:test + shared/logic.js）'
node --test tests/logic.test.js

echo
echo '[2/8] M2a API / 数据层 / 对局存档单测（进程内临时 SQLite）'
node --test tests/api.test.cjs

echo
echo '[3/8] 在线断连复现与心跳保活模拟（进程内 + 空闲超时代理）'
node --test tests/heartbeat-sim.cjs

echo
echo '[4/8] 断线重连恢复棋局（房间保留期）'
node --test tests/reconnect-state.test.cjs

echo
echo '[5/8] 空闲房间清理（防挂着不退出泄漏）'
node --test tests/room-cleanup.test.cjs

echo
echo '[6/8] 在线悔棋（服务端权威，撤回自己刚走的一步）'
node --test tests/online-undo.test.cjs

echo
echo '[7/8] 对局历史权限（仅参赛双方可见）'
node --test tests/history-auth.test.cjs

echo
echo '[8/8] 界面端到端（headless Chrome + CDP，含断线自动重连）'
