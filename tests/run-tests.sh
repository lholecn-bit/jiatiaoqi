...
echo '[1/7] 规则逻辑单测（node:test + shared/logic.js）'
node --test tests/logic.test.js

echo
echo '[2/7] M2a API / 数据层 / 对局存档单测（进程内临时 SQLite）'
node --test tests/api.test.cjs

echo
echo '[3/7] 在线断连复现与心跳保活模拟（进程内 + 空闲超时代理）'
node --test tests/heartbeat-sim.cjs

echo
echo '[4/7] 断线重连恢复棋局（房间保留期）'
node --test tests/reconnect-state.test.cjs

echo
echo '[5/7] 空闲房间清理（防挂着不退出泄漏）'
node --test tests/room-cleanup.test.cjs

echo
echo '[6/7] 在线悔棋（服务端权威，撤回自己刚走的一步）'
node --test tests/online-undo.test.cjs

echo
echo '[7/7] 界面端到端（headless Chrome + CDP，含断线自动重连）'
